...
export const contact= catchAsyncError(async(req,res,next)=>{
    ...
    if(!name||!email|| !message) return next(new ErrorHandler("All fields are mendotery",400));
...
export const courseRequest= catchAsyncError(async(req,res,next)=>{
    ...
    if(!name||!email|| !course) return next(new ErrorHandler("All fields are mendotery",400));
    const to= process.env.MY_MAIL;
    const subject= "Requesting for a course Course";
    const text = ` I am ${name} and my Email is ${email}.\n${course}`;
    await sendEmail(to,subject,text);
...
export const getDashbordStats= catchAsyncError(async(req,res,next)=>{
